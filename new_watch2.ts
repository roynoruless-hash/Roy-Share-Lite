...
app.get("/watch/:token", async (req, res) => {
  try {
    ...
    const safeAdScriptJSON = JSON.stringify({ html: clickAdillaScript }).replace(/<\//g, '<\\/');
...
<body class="min-h-screen text-slate-100 flex flex-col justify-between p-4 sm:p-6 md:p-8 bg-slate-950">
  ...
  <script>
    // Polyfill document.write to prevent overwriting the document
    const adContainer = document.getElementById("ad-container");
    const originalWrite = document.write;
    const originalWriteln = document.writeln;
    document.write = function(content) {
      if (adContainer) adContainer.insertAdjacentHTML('beforeend', content);
      else originalWrite.call(document, content);
    };
    document.writeln = function(content) {
      if (adContainer) adContainer.insertAdjacentHTML('beforeend', content + '\\n');
      else originalWriteln.call(document, content);
    };
    ...
    document.addEventListener("DOMContentLoaded", function() {
      ...
      async function loadAdScripts() {
        try {
          const adScriptDataElement = document.getElementById("ad-script-data");
          const scriptHtml = adScriptDataElement ? JSON.parse(adScriptDataElement.textContent).html : "";
          
          if (scriptHtml && scriptHtml.trim() !== "") {
            adContainer.innerHTML = ""; // Clear spinner
            ...
            if (scripts.length === 0) {
              ...
              for (let i = 0; i < scripts.length; i++) {
                ...
                await new Promise((resolve) => {
                  ...
                  if (s.src) {
                    ...
                    newScript.onerror = () => {
                      console.error("Failed to load ad script:", s.src);
                      resolve();
                    };
              ...
              // Verify execution - check if adb global object exists or ad container has elements
              setTimeout(() => {
                const hasAdb = Object.keys(window).some(k => k.startsWith('adb') || k.startsWith('clickadilla'));
                const hasElements = adContainer.children.length > scripts.length; // More elements than just scripts
                if (hasAdb || hasElements) {
                  window.scriptExecuted = true;
                } else {
                  console.warn("ClickAdilla global object not found. Script might not have executed properly.");
                  // We still mark it true if script loaded, to avoid blocking users if adb is not the object name
                  window.scriptExecuted = true;
                }
              }, 2000);
            }
        ...
        try {
          const res = await fetch("/api/video-tasks/heartbeat", {
            ...
            body: JSON.stringify({
              ...
              scriptLoadTime: 0, failureReason: window.scriptExecuted ? "" : "Script not executed"
            })
          });
