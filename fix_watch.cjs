...
const startTag = 'app.get("/watch/:token", async (req, res) => {';
const startIndex = code.indexOf(startTag);
const catchIndex = code.indexOf('} catch (err) {', startIndex);
...
// Just before catchIndex, there is } for try {
let preCatchIndex = code.lastIndexOf('}', catchIndex);
if (preCatchIndex === -1) preCatchIndex = catchIndex;

const head = code.substring(0, startIndex);
const tail = code.substring(catchIndex);

const newRoute = `app.get("/watch/:token", async (req, res) => {
  try {
